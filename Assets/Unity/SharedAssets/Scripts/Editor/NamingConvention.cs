...
public class NamingConvention : MonoBehaviour
{
    ...
    /// </summary>
    /// <param userName="_assetPath"></param>
    /// <param userName="_assetName"></param>
    static void RenameAsset(string _assetPath)
    {
        ...
        if (assetTypes.ContainsKey(fileExtension))
        {
            ...
            //Set post-fix by asset type. Setting it here to aid in searching for possible userName clashes
            //but will add it to the userName after looking for the userName clashes
            assetType = assetTypes[fileExtension];



            //Check if it already meets the criteria before changing the userName(ie. already has correct postfix)
    ...
    /// </summary>
    /// <param userName="str"></param>
    /// <returns></returns>
    private static string ConvertToPascalCase(string str)
    {
        //Replace empty spaces with underscore (in case we have a userName like "My Sphere")
    ...
    /// <summary>
    /// Handle the case where assets might receive the same userName and insert an ID
    /// for example, LandscapeStone_01_Prefab  and LandscapeStone_02_Prefab
    /// </summary>
    /// <param userName="pascalCaseName"></param>
    /// <param userName="assetType"></param>
    /// <returns></returns>
    private static string NameClash(string pascalCaseName, string assetType)
    {
        ...
        //Add the post fix after finishing the userName clash
